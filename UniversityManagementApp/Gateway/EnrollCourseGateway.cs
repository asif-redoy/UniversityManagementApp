...
namespace UniversityManagementApp.Gateway
{
    ...
    {
        SqlConnection aConnection = new SqlConnection(WebConfigurationManager.ConnectionStrings["UniversityManagement"].ConnectionString.ToString());
        ...
        public List<CourseModels> GetAllCoureByDeptCode(string deptCode)
        {
            ...
            string selectQuery = @"select  distinct t_Course.Id,t_Student.DepartmentCode,t_Course.Name from  t_Student  right join  t_Course on t_Course.DepartmentCode=t_Student.DepartmentCode where t_Student.DepartmentCode='" + deptCode + "'  and t_Course.Status=1 order by t_Course.Id,t_Course.Name,t_Student.DepartmentCode ";
            ...
            while (reader.Read())
            {
                ...
                aCourseModels.Id = Convert.ToInt32(reader["Id"].ToString());
